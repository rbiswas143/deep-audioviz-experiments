...
def pad_mfccs(x, num_net_scale_downs, total_segments, num_mfcc, num_mfcc_frames):
    divisor = 2 ** num_net_scale_downs

    num_pad_mfcc_frames = (
        0 if num_mfcc_frames % divisor == 0 else (int(num_mfcc_frames / divisor) + 1) * divisor - num_mfcc_frames)
    x_pad_frames = np.zeros((total_segments, num_mfcc, num_pad_mfcc_frames, 1))
    ...
    x_pad_mfcc = np.zeros((total_segments, num_pad_mfcc, num_mfcc_frames_new, 1))
